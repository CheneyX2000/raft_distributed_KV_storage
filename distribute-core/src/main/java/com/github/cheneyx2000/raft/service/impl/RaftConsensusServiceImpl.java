package com.github.cheneyx2000.raft.service.impl;

import com.github.cheneyx2000.raft.RaftNode;
...
public class RaftConsensusServiceImpl implements RaftConsensusService {
    ...
    public RaftProto.VoteResponse preVote(RaftProto.VoteRequest request) {
        ...
        try {
            ...
            if (!ConfigurationUtils.containsStorageServer(raftNode.getConfig(), request.getServerId())) {
    ...
    public RaftProto.VoteResponse requestVote(RaftProto.VoteRequest request) {
        ...
        try {
            ...
            if (raftNode.getVotedFor() == 0 && logIsOk) {
                ...
                raftNode.getRaftLog().updateMeta(raftNode.getCurrentTerm(), raftNode.getVotedFor(), null, null);
                responseBuilder.setGranted(true);
                responseBuilder.setTerm(raftNode.getCurrentTerm());
            }
        ...
        try {
            ...
            if (raftNode.getLeaderId() == 0) {
                ...
                LOG.info("new leaderId={}, conf={}",
                        ...
                        PRINTER.printToString(raftNode.getConfig()));
            }
            ...
            // raftNode.getRaftLog().updateMeta(raftNode.getCurrentTerm(),
    ...
    public RaftProto.InstallSnapshotResponse installSnap(RaftProto.InstallSnapshotRequest request) {
        ...
        try {
            ...
            // file name might be a relative path, like topic/0/message.txt
    ...
    private void advanceCommitIndex(RaftProto.AppendEntriesRequest request) {
        ...
        raftNode.getRaftLog().updateMeta(null, null, null, newCommitIndex);
            ...
            for (long index = raftNode.getLastAppliedIndex() + 1; index <= raftNode.getCommitIndex(); index++) {
                ...
                if (entry != null) {
                    if (entry.getType() == RaftProto.EntryType.ENTRY_TYPE_DATA) {
                        ...
                        raftNode.applyConfig(entry);
                    }
                }
                raftNode.setLastAppliedIndex(index);
            }
        }
        // In Follower-Read (Read Index) mode，Follower would enter wait condition, wait
        // for log to be applied to copy state machine, so need to wake the awaiting
        // threads
    ...
    public RaftProto.GetLeaderCommitIndexResponse getLeaderCommitIndex(RaftProto.GetLeaderCommitIndexRequest request) {
        RaftProto.GetLeaderCommitIndexResponse.Builder responseBuilder = RaftProto.GetLeaderCommitIndexResponse
                .newBuilder();
        raftNode.getLock().lock();
        long commitIndex;
        try {
